...
# NX300 CT3 full register baseline v1.3 - sensor read-only.
...
probe_log="$probe_root/CT3_FULL_V1_3.LOG"
trace_log="$probe_root/CT3_FULL_REGISTER_TRACE_V1_3.LOG"
...
{
    echo "=== CT3 FULL REGISTER PROBE V1.3 START ==="
    date
...

{
    ...
    echo "read_results=$(grep -c 'CIS_READ ADDRESS' "$trace_log" 2>/dev/null)"
    echo "=== CT3 FULL REGISTER PROBE V1.3 END ==="
    date
} >> "$probe_log" 2>&1
