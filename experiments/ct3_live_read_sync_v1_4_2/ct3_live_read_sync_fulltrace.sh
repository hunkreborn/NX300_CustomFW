...
# CT3 live read synchronization test v1.4.2 - unfiltered strace.
...
probe_log="$probe_root/CT3_LIVE_READ_SYNC_V1_4_2.LOG"
trace_log="$probe_root/CT3_LIVE_READ_SYNC_TRACE_V1_4_2.LOG"
...
trap cleanup_fulltrace EXIT INT TERM

{
    echo "=== CT3 LIVE READ SYNC V1.4.2 START ==="
    date
...
: > "$trace_log"
# No syscall filter: required by the old ARM strace to retain entry/exit sync.
/usr/bin/strace -p "$shell_tid" -s 512 -o "$trace_log" &
strace_pid=$!
sleep 1
...

{
    ...
    echo "=== CT3 LIVE READ SYNC V1.4.2 END ==="
    date
} >> "$probe_log" 2>&1
exit 0
